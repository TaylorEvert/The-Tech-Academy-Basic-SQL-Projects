...
/* PROCEDURES ------------------- LIBRARY DATABASE */
	...
	CREATE PROCEDURE totalNumber
	AS
	SELECT Library_Branch.BranchName, COUNT(Book_Loans.DateOut) AS BooksLoaned
	FROM (Library_Branch
	INNER JOIN Book_Loans ON Library_Branch.BranchID = Book_Loans.BranchID)
	GROUP BY Library_Branch.BranchName;
	EXEC dbo.totalNumber;
	 /* For each library branch, retrieve the branch name and the total number of books loaned out from that branch */

	CREATE PROCEDURE borrowerList5
	AS
	SELECT COUNT(Book_Loans.DateOut) AS CheckedBooks, Borrower.BorrowerName, Borrower.BorrowerAddress
	FROM (Book_Loans
	INNER JOIN Borrower ON Book_Loans.CardNo = Borrower.CardNo)
	GROUP BY Borrower.BorrowerName, Borrower.BorrowerAddress HAVING COUNT(Book_Loans.DateOut)>5;
	...
	AS 
	SELECT Book_Copies.Number_of_Copies,  Books.Title, Book_Authors.AuthorName, Library_Branch.BranchName
	FROM (((Book_Copies
	INNER JOIN Books ON Book_Copies.BookID = Books.BookID)
